...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, mainWindow):
        ...
        self.address_label.setText(_translate("MainWindow", "Хост: X.X.X.X"))
        self.port_label.setText(_translate("MainWindow", "Порт: XXXX"))
        self.clients_label.setText(_translate("MainWindow", "Игроки:"))
...
def start_server():
    ...
    ui.address_label.setText("Хост: " + HOST_ADDR)
    ui.port_label.setText("Порт: " + str(HOST_PORT))
...
def stop_server():
    ...
    player_data = []
    ui.address_label.setText("Хост: X.X.X.X")
    ui.port_label.setText("Порт: XXXX")
...
# получаем сообщение от текущего клиента и отправляем его другим
def send_receive_client_message(client_connection, client_ip_addr):
    ...
    update_client_names_display(clients_names)  # обновляем отображение имен клиентов

    if len(clients) > 1:
        sleep(1)
        symbols = ["O", "X"]

        # отправка имени и символов о или х противнику
        clients[0].send(("Другой игрок:" + clients_names[1] + "symbol:" + symbols[0]).encode('utf-8'))
        clients[1].send(("Другой игрок:" + clients_names[0] + "symbol:" + symbols[1]).encode('utf-8'))

    while True:
        # получаем выбор игрока из полученных данных
        data = client_connection.recv(4096).decode('utf-8')
        if not data:
            break

        # данные о ходе, отправка другому игроку
    ...
    # находим индекс клиента и удаляем из списка клиентов и списка подключений
...
# возвращает индекс текущего клиента в списке клиентов
...
# при подключении нового клиента или отключении подключенного клиента обновляем список клиентов
